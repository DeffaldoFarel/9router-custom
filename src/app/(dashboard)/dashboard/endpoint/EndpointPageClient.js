...
export default function APIPageClient({ machineId }) {
  ...
  const handleSaveAllowedModels = async (id, patterns) => {
    try {
      console.log("[AllowedModels] Saving:", { id, patterns });
      const res = await fetch(`/api/keys/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (res.ok) {
        console.log("[AllowedModels] Saved successfully:", data.key?.allowedModels);
        setKeys(prev => prev.map(k => k.id === id ? { ...k, allowedModels: data.key?.allowedModels || [] } : k));
        setShowModelAccessModal(false);
        setModelAccessKey(null);
      } else {
        console.error("[AllowedModels] Save failed:", res.status, data);
        alert(`Failed to save: ${data?.error || "Unknown error"}`);
      }
    } catch (error) {
      console.error("[AllowedModels] Error saving:", error);
  ...
  return (
    <div className="flex flex-col gap-8">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {key.allowedModels && key.allowedModels.length > 0 && (
                      <span className="inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded bg-primary/10 text-primary font-medium">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-surface-2 border border-border-subtle rounded-lg p-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-text-main mb-0.5">Configure model access after creation</p>
                <p className="text-xs text-text-muted">After creating the key, click the tune icon to select allowed models using the visual picker.</p>
              </div>
            </div>
          </div>
      ...
      {/* Edit Allowed Models Modal - Visual Model Picker */}
      <ApiKeyModelAccessModal
        isOpen={showModelAccessModal}
        keyName={modelAccessKey?.name || ""}
        currentAllowedModels={modelAccessKey?.allowedModels || []}
        onClose={() => {
          setShowModelAccessModal(false);
          setModelAccessKey(null);
        }}
        onSave={(patterns) => handleSaveAllowedModels(modelAccessKey?.id, patterns)}
